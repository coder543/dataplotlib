...
fn main() {
    ...
    while let Some(event) = window.next() {
        ...
        event.update(|_| {
            ...
            let time = start.elapsed().subsec_nanos() as f64 / 1_000_000_000.0;
            if (time > plot_time && (time - plot_time).abs() >= (1.0/60.0)) || (plot_time - time).abs() > (1.0/60.0) {
                plot_time = time;
            }
            let min_x = plot_time * 2.0 * PI;
            let max_x = min_x + PI * 2.0;
            let min_y = -1.0;
            let max_y = 1.0;
